...
var
  BtnTypeName: array[TNavigateBtn] of PChar = ('FIRST', 'PRIOR', 'NEXT',
    'LAST', 'INSERT', 'DELETE', 'EDIT', 'POST', 'CANCEL', 'REFRESH'{$IFDEF DELPHI_XE2}, 'APPLYUPDATES', 'CANCELUPDATES'{$ENDIF});
  ...
  SDBN_POST, SDBN_CANCEL, SDBN_REFRESH{$IFDEF DELPHI_XE2}, SDBN_APPLYUPD, SDBN_CANCELUPD{$ENDIF} : TBitmap;
...
{$IFDEF DELPHI_XE2}
  else if TypeName = 'APPLYUPDATES' then Btn.Glyph.Assign(SDBN_APPLYUPD)
  else if TypeName = 'CANCELUPDATES' then Btn.Glyph.Assign(SDBN_CANCELUPD)
{$ENDIF}
end;
...
begin
  if (DataSource <> nil) and (DataSource.State <> dsInactive) then begin
    ...
    with DataSource.DataSet do case Index of
      ...
      nbPost: Post;
      nbRefresh: Refresh;
      nbDelete: if not FConfirmDelete or (sMessageDlg(SDeleteRecordQuestion, mtConfirmation, mbOKCancel, 0) <> idCancel) then Delete;
    end;
  end;
