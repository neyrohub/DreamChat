...
var
  ...
  begin
    ...
    if CurColumn.Title.Caption <> '' then begin
      for i := ACol + 1 to Columns.Count - 1 do begin
        ...
        if UpperCase(s1) = UpperCase(s2) then inc(Result) else Break;
      end;
    end;
  end;
  ...
  var
    i : integer;
  begin
    i := WordCount(Columns[ACol].Title.Caption, [FLevelDelimiterChar]);
    if i = 1 then Result := FHeaderHeight - 1 else Result := 1;
  end;
  ...
  var
    ...
    Size : TSize;
  begin
    if (dgRowLines in options) or (dgColLines in options) then begin
      iSkinNdx := FCommonData.SkinManager.GetSkinIndex(s_ColHeader);
      if iSkinNdx < 0 then iSkinNdx := FCommonData.SkinManager.GetSkinIndex(s_Button);
    end
    else iSkinNdx := -1;
    if iSkinNdx >= 0
      then PaintItem(iSkinNdx, s_ColHeader, CI, True, 0, ARect, Point(0, 0), Canvas.Handle, FCommonData.SkinManager)
      else FillDC(Canvas.Handle, ARect, SkinData.SkinManager.GetGlobalColor);
    if AllowArrow and FActiveRowSelected then begin
      if (DataLink <> nil) and DataLink.Active and DataLink.Editing then begin
        Canvas.Font.Assign(Font);
        if iSkinNdx >= 0 then Canvas.Font.Color := FCommonData.SkinManager.gd[iSkinNdx].Props[0].FontColor.Color else Canvas.Font.Color := FCommonData.SkinManager.GetGlobalFontColor;
        ...
        if iSkinNdx >= 0 then Canvas.Font.Color := FCommonData.SkinManager.gd[iSkinNdx].Props[0].FontColor.Color else Canvas.Font.Color := FCommonData.SkinManager.GetGlobalFontColor;
        DrawColorArrow(Canvas, Canvas.Font.Color, ARect, BF_RIGHT);
      end;
    end;
  end;
  ...
  begin
    ...
    if (MasterCol = nil) then begin
      if not bSkinned then Canvas.FillRect(ARect);
      Exit;
    end;
    ...
    for CurLevel := 0 to FHeaderHeight - 1 do begin
      // Check dependencies
      if TacColumnTitle(MasterCol.Title).IsCaptionStored
        then CurCaption := GetCaptionLevel(Caption, CurLevel, FLevelDelimiterChar)
        else if CurLevel = 0 then CurCaption := Caption else CurCaption := '';
      ...
      if CurCaption <> '' then begin
        if lvCheckLeft then begin
          lvLeftBorder := True;
          lvShowCaption := True;
          if (Column.Index = LeftCol - IndicatorOffset) or (CurLevel = (CaptionDepth - 1)) then lvCheckLeft := False else begin
            ...
            while lvTmpColIndex >= 0 do begin
              ...
              if UpperCase(tmpCaption) <> UpperCase(CurCaption) then begin
                ...
                if dgColLines in Options then inc(lvCaptionXOffset);
                dec(lvTmpColIndex)
              end;
            end;
          end;
        ...
        if lvCheckRight then begin
          lvRightBorder := True;
          if (Column.Index = MasterCol.Collection.Count - 1) or (CurLevel = (CaptionDepth - 1)) then lvCheckRight := False else begin
            lvTmpColIndex := Column.Index + 1;
            lvTmpCol := TColumn(MasterCol.Collection.Items[lvTmpColIndex]);
            tmpCaption := GetCaptionLevel(lvTmpCol.Title.Caption,CurLevel,FLevelDelimiterChar);
      ...
      else if Caption <> '' then lvShowCaption := False;
      ...
      if not bSkinned then begin
        Canvas.FillRect(TextRect);
        // Draw borders for level
        CellFlag := BDR_RAISEDINNER;
        if (FTitleButtonDown = Column.Index) and (CurLevel >= CaptionDepth-1) then CellFlag := BDR_SUNKENINNER;
      ...
      if lvShowCaption then begin
        ...
        if bSkinned then begin
          ...
          if ACol <> lvTmpColIndex then begin
          ...
          if iMaxRow <> CurLevel then begin
          ...
          if iSkinNdx >= 0 then begin
            PaintItem(iSkinNdx, s_ColHeader, CI, True, 0, R, Point(0, 0), Canvas.Handle, FCommonData.SkinManager);
            Canvas.Font.Color := FCommonData.SkinManager.gd[iSkinNdx].Props[0].FontColor.Color;
          end
          else begin
            FillDC(Canvas.Handle, R, SkinData.SkinManager.GetGlobalColor);
      ...
      else if not bSkinned then begin
        if CurCaption = '' then begin
          if not bSkinned then WriteText(Canvas, TextRect, FrameOffs, FrameOffs, '', lvCaptionAligment, IsRightToLeft) // mean there is coninue of previous column
        end
        else begin
          if dgColLines in Options then begin
            dec(TextRect.Left,1);
