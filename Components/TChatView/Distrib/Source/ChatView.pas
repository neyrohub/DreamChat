  ...
  if not ShareContents then begin
      ...
      for i:=0 to lines.Count-1 do begin
        if TLineInfo(lines.objects[i]).StyleNo = -3 then { image}
          begin
//            TLineInfo(lines.objects[i]).gr.Free;//я закомм
   ...
   if not (OnlyResized and (TextWidth=OldTextWidth)) then begin
     ...
     for i:=StartLine to lines.Count-1 do begin
       ...
       if not (((StyleNo = cvsPicture) and (not (cvdoImages in DisplayOptions))) or
          ...
          (((StyleNo = cvsBullet) or (StyleNo = cvsHotspot))and(not (cvdoBullets in DisplayOptions)))) then
...
{-------------------------------------}
procedure TChatView.FormatLine(no: Integer; var x,baseline,prevdesc,prevabove:Integer; Canvas: TCanvas;
                         var sad: TScreenAndDevice);
       ...
       while sourceStrPtrLen>0 do begin
         ...
         GetTextExtentExPoint(Canvas.Handle,  sourceStrPtr,  sourceStrPtrLen, Width-x,
                              {$IFDEF ChatViewDEF4}
                                @max, nil,
                              {$ELSE}
                                max, arr[0],//было так но не компилится
                              {$ENDIF}
                              sz);
         if max=0 then max := 1;
...
//я           max,arr[0],
         ...
         if (StyleNo=cvsJump1) or (StyleNo=cvsJump2) then begin
           jmpinfo := TJumpInfo.Create;
...
{-------------------------------------}
function TChatView.GetFirstVisible(TopLine: Integer): Integer;
begin
   Result := GetDrawLineNo(TopLine,gdlnFirstVisible);
end;
{-------------------------------------}
function TChatView.GetFirstLineVisible: Integer;
var v: Integer;
begin
...
{-------------------------------------}
function TChatView.GetLastLineVisible: Integer;
var v: Integer;
begin
...
{-------------------------------------}
function TChatView.GetDrawLineNo(BoundLine: Integer; Option: Integer): Integer;
var
    a,b,mid: Integer;
begin
...
begin
 ...
 then begin
    ...
    if (csDesigning in ComponentState) then
      Canvas.TextOut(ClientRect.Left+1, ClientRect.Top+1, 'ChatView v0.5.1 (www.tChatView.com)')
    else
...
begin
  ...
  for i:=FirstLine to FirstLine+Count-1 do begin
    if TLineInfo(lines.objects[i]).StyleNo = -3 then { image}
      begin
//        TLineInfo(lines.objects[i]).gr.Free;//я закомментил
        TLineInfo(lines.objects[i]).gr := nil;
      end;
    if TLineInfo(lines.objects[i]).StyleNo = -5 then {control}
      begin
