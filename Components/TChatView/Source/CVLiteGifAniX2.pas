...
//и все упадет.
interface
uses windows, classes, Graphics, ExtCtrls, sysutils,
     litegifx2;
...
//    constructor Create(MS: TMemoryStream; FileName:String);
    constructor Create(GifImage:TGif; FrameBackGroundColor:TColor);
...
{-------------------------------------}
//constructor Create(MS: TMemoryStream; FileName:String);
constructor TGifAni.Create(GifImage:TGif; FrameBackGroundColor:TColor);
var n: integer;
    TempBitmap: TBitmap;
...
//  StartForm.SetMax(FGifImage.ImageCount);
  TempBitmap.Canvas.Brush.Color := FrameBackGroundColor;
//  TempBitmap.TransparentMode := ;
//внимание! вот тут похимичеть ^^^^^^^^ с методом наложения битмэпа
