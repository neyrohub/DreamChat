...
{    case ScrollCode of
    ...
    case ScrollCode of
      ...
      SB_PAGEUP: SetHPos(HPos - 1);
      SB_PAGEDOWN: SetHPos(HPos + 1);
      SB_THUMBPOSITION: SetHPos(Pos);
      SB_THUMBTRACK: if FTracking then SetHPos(Pos);
      SB_TOP: SetHPos(0);
      SB_BOTTOM: SetHPos(XSize);
...
//      SB_PAGEUP: SetVPos(VPos - 10);так было
//      SB_PAGEDOWN: SetVPos(VPos + 10);так было
      SB_PAGEUP: SetVPos(VPos - 1);
      SB_PAGEDOWN: SetVPos(VPos + 1);
...
begin
  ...
  if OldPos - VPos <> 0 then
    ...
    if FFullRedraw then
      ...
      begin
      ScrollWindowEx(Handle, 0, (OldPos-VPos)*SmallStep, nil, @r, 0, nil, SW_ERASE
