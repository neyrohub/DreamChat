...
const
  cvVersion     = 'TChatView v0.30 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
procedure TChatView.AddControl(ctrl: TWinControl; center: Boolean); { do not free ctrl! }
var info: TContainerInfo;
begin
...
for n := 0 to DrawContainers.Count - 1 do
  ...
  if li.StyleNo = -5 then
    begin
    if (n >= LastCont) or (n <= FirstCont) then
      begin
...
{===============================================================================}
   if (no = -5)  then // Control
     ...
     //высоту контрола
     TControl(li.gr).Top := dli.Bottom^ - dli.Height - VPos * SmallStep;
     TControl(li.gr).Left := dli.Left;

     self.OnDebug('paint: TControl(li.gr).Top =' + Inttostr(TControl(li.gr).Top));
     end;
{===============================================================================}
...
//Ondebug(s);
       ...
       info.pDrawLineInfo := DrawLineInfo;
       DrawContainers.AddObject('', info);
       TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Top := -1000;//info.Bottom^;
s := {'TControl(TContainerInfo(ContStorage}'.Objects[' +
     inttostr(ContNum) +
//     ']).gr).Top =' + inttostr(info.Bottom^);
     ']).gr).Top =' + inttostr(TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Top);
Ondebug(s);
