...
const
  cvVersion     = 'TChatView v0.29 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
{поразмыслить над объединением их в один класс}
  ...
  private
    ...
    FDrawGifAni: TDrawGifAni;
    FBeginGifAni: TBeginGifAni;
...
//DrawFrame;
inherited;
//if @FDrawGifAni <> nil then FDrawGifAni();
end;
...
//  Paint;
//If @self.FDrawGifAni <> nil then SetGifAniCanvas(Canvas);
end;
{-------------------------------------}
...
begin
  ...
  if not ShareContents then
    ...
    for i := 0 to ContStorage.Count - 1 do
      ...
      if TContainerInfo(ContStorage.objects[i]).StyleNo = -5 then {control}
        begin
...
{-------------------------------------}
{хотел проверять присвоен ли обработчик.... но делать надо не так
...
//  info.imgNo := imgNo;
  info.SameAsPrev := not FromNewLine;
  ContStorage.AddObject('', info);
  //запоминаем метод для отрисовки GIF'ов
//  if @FDrawGifAni = nil then FDrawGifAni:= GifAniObject.DrawFrame;
//  if @FBeginGifAni = nil then FBeginGifAni:= GifAniObject.BeginAnimate;
...
for i := 0 to ContStorage.Count-1 do
  ...
  if TContainerInfo(ContStorage.objects[i]).StyleNo = -5 then
    if m < TControl(TContainerInfo(ContStorage.objects[i]).gr).Width then
      m := TControl(TContainerInfo(ContStorage.objects[i]).gr).Width;
...
//        TControl(li.gr).Tag := dli.Top^;
        TControl(li.gr).Tag := dli.Bottom^ - dli.Height;
...
//FDebugText := inttostr(VPOS) + ' GetFirstVisibleContainer = ' + inttostr(result);
...
//FDebugText := 'DrawContainer = ' + inttostr(FindItemAtPos(x, y));
//self.OnDebug(FDebugText);
end;
{-------------------------------------}
...
for n := 0 to DrawContainers.Count - 1 do
  ...
  if (Y >= dli.pDrawLineInfo.BaseLine - dli.Height) and (Y <= dli.pDrawLineInfo.BaseLine) and
...
var
    ...
    sz: TSIZE;
begin
  if DrawContainers.Count = 0 then begin
     No := -1;
     exit;
  end;
...
var n, FirstCont, LastCont : Cardinal;
    dli:TDrawContainerInfo;
    li: TContainerInfo;
    r :TRect;
begin
//Для чего все это:
FirstCont := FirstVisibleContainer;
LastCont := LastVisibleContainer;
for n := 0 to DrawContainers.Count - 1 do
  begin
  ...
  if li.StyleNo = -8 then
    begin
    if (n >= FirstCont) and (n <= LastCont) then
      begin
...
//короче проблема именно в этих -13!
//при анимации на экран выводятся те смайлики которые уже за экраном
end;
{-------------------------------------}
...
//Внимание! Далее мы создает BufferVirtCanv высотой в 1 пиксел!
...
//в качестве параметров передавать 1 пиксел из r. Читать выше!!!!
 for i := GetFirstVisibleContainer to GetLastVisibleContainer do
   begin
   {в i в цикле прокручиваются номера строк, видимых на экране}
   {DrawContainers - это массив строк, которые ВИДНЫ на экране типа TStringList}
   dli := TDrawContainerInfo(DrawContainers.Objects[i]);
...
//   if dli.top^ > r.Bottom then lastline := True;
   ...
   if no >= 0 then
     ...
     if (StartNo > i) or (EndNo < i) then
       begin
       canv.Font.Color := textcolor;
       //весь текст выводится именно здесь
...
//     TGifAni(li.gr).MirrorImagesY[li.imgNo] := dli.Bottom^ - dli.Height - yshift;
     //а теперь вот так... ничего лучше не придумал, зато работает
     //Они полетели на Землю другим путем :-)
     TGifAni(li.gr).MirrorImagesY[li.imgNo] := dli.Bottom^ - dli.Height - VPos * SmallStep;
     TGifAni(li.gr).MirrorImagesX[li.imgNo] := dli.Left;
     TGifAni(li.gr).DestCanvas := BufferVirtCanv.Canvas;
     TGifAni(li.gr).DrawFrame(li.imgNo, xshift, dli.Bottom^ - dli.Height - yshift);
     end;
{===============================================================================}
   if (no = -7)  then { gif }
     begin
...
{===============================================================================}
   if (no = -5)  then { Control }
     begin
     TControl(TContainerInfo(ContStorage.Objects[li.imgNo]).gr).Top := dli.Bottom^;
     TControl(TContainerInfo(ContStorage.Objects[li.imgNo]).gr).Left := dli.Left;
     end;
{===============================================================================}
...
//     TImageList(li.gr).Draw(canv, dli.Left-xshift, dli.top^ -yshift, li.imgNo);
     TImageList(li.gr).Draw(canv, dli.Left - xshift, dli.Bottom^ - dli.Height - yshift, li.imgNo);
...
{===============================================================================}
   if (no = -3)  then
     begin { graphics }
     canv.Draw(dli.Left-xshift, dli.Bottom^  - yshift, TGraphic(li.gr));
     continue;
     end;
{===============================================================================}
...
{++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++}
     ...
     self.TextHeight := TDrawLineInfo(DrawLinesInfo.Objects[Line - 1]).BaseLine{b} + d + 1;
     if TextHeight div SmallStep > 30000 then
       self.SmallStep := self.TextHeight div 30000;
     AdjustJumpsCoords;
     end
   else
...
{MessageBox(0, 'info.Bottom^',
              PChar(Inttostr(info.Bottom^) + '  ' + Inttostr(TContainerInfo(ContStorage.Objects[ContNum]).imgNo)),
              mb_ok);}
   ...
   -5: { Control }
     begin
       ctrlw       := TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Width;
       ctrlh       := TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Height;
       ctrlw       := MulDiv(ctrlw, sad.ppixDevice, sad.ppixScreen);
       ctrlh       := MulDiv(ctrlh, sad.ppiyDevice, sad.ppiyScreen);
       info        := TDrawContainerInfo.Create;
       info.Width  := ctrlw;
      ...
      TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Top := info.Bottom^;
      TControl(TContainerInfo(ContStorage.Objects[ContNum]).gr).Left := x;
...
//                    'DrawLineInfo.LineNumber   baseline', mb_ok);
        ...
        if max = 0 then max := 1;//проблемы с зацикливанием, если ничего не помещается

        if max < sourceStrPtrLen then
        //если max меньше, чем длина текста (т.е. текст НЕ ПОМЕЩАЕТСЯ до конца канваса)
          begin
          //пробуем получить в этом куске указатель ПОСЛЕДНИЙ пробел
