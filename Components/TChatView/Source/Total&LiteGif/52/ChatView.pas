...
const
  cvVersion     = 'TChatView v0.52 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
type
  ...
  {Объект-контейнер, который содержит инфу о том в какие координаты X,Y
   ...
   быть текстом, GIF, Control и т.д.}
     ...
     LinkId: integer;//раньше был адрес на объект TLinkInfo, но при сравнении
...
//    procedure AdjustChildrenCoords;
  ...
  public
    ...
    Function DeleteLink(LinkId:Integer):boolean;//удаляем ссылку из массива ссылок
...
var dli: TDrawContainerInfo;
    ...
    li: TContainerInfo;
begin
//изменztv стиль отрисовки объекта (теперь он выделен)
if ContNum >= 0 then
  begin
...
//  SetStyleOfLinkObject(LastLinkMovedAbove, 5);
  InvalidateLinkObject(LastLinkMovedAbove);
  LastLinkMovedAbove := -1;
  end;
...
if ItemAtPos > 0 then
  ...
  if TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId >= 0 then
    ...
    Cursor := FStyle.JumpCursor;
    FDebugText := FDebugText + TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId]).LinkText + #10#13;
...
//    SetStyleOfLinkObject(ItemAtPos, 3);
  ...
  for i:=0 to Source.ContStorage.Count-1 do begin
    ...
    case li.StyleNo of
      ...
      -6: AddBullet(li.imgNo, TImageList(li.gr), not li.SameAsPrev);
        ...
        begin
          if li.Center then
               AddCenterLine(Source.ContStorage[i], li.StyleNo, -1)//!!!!!!!!!внимание!!!
          else
             if li.SameAsPrev then
                Add(Source.ContStorage[i], li.StyleNo, -1)//!!!!!!!!!внимание!!!
                //тут потом посмотреть -1 временно!!!
             else
                AddFromNewLine(Source.ContStorage[i], li.StyleNo, -1)//!!!!!!!!!внимание!!!
        end;
    end;
  end;
end;
