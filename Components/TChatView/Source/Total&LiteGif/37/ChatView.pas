...
const
  cvVersion     = 'TChatView v0.37 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
//    procedure CMInvalidate(var Message: TMessage); message CM_INVALIDATE;
    ...
    function FindLineItemAtPos(X,Y: Integer): Integer;//ищет по всему отображаемому массиву (форматированных) объектов
    function FindLineItemAtScreenPos(ScrX, ScrY: Integer): Integer;//ищет по всему отображаемому массиву (форматированных) объектов
    function FindSymbolAtScreenPos(ScrX, ScrY: Integer): String;
    procedure SetGifAniCanvas(DestionationCanvas: TCanvas);
    procedure CMMouseLeave(var Message: TMessage); message CM_MOUSELEAVE;
    procedure SetSelBounds(SelStartX, SelStartY, SelEndX, SelEndY: Integer);
...
{    FSelStartX := x;
    ...
    SetSelBounds(FSelStartX, FSelStartY, FSelEndX, FSelEndY);
//    SetSelBounds(X, Y);
    selection := true;
    invalidate;
    end;
...
if FindItemAtScreenPos(x, y) > 0 then
  ...
  FDebugText := 'MouseMove' + #10#13 + 'Под курсором нет контейнера' +
                #10#13 + 'x = ' + inttostr(x) + '   y = ' + inttostr(y);
if FindLineItemAtScreenPos(x, y) > 0 then
  FDebugText := FDebugText +
                #10#13 + 'MaxHeight =' + inttostr(TDrawContainerInfo(DrawContainers.Objects[FindLineItemAtScreenPos(x, y)]).pDrawLineInfo.MaxHeight) +
                #10#13 + 'GetFirstContainerInLine = ' + inttostr(GetFirstContainerInLine(FindLineItemAtScreenPos(x, y))) +
                '   GetLastContainerInLine = ' + inttostr(GetLastContainerInLine(FindLineItemAtScreenPos(x, y))) +
...
{-------------------------------------}
function TChatView.FindLineItemAtScreenPos(ScrX,ScrY: Integer): Integer;
begin
result := FindLineItemAtPos(ScrX + HPos,ScrY + VPos);
end;
{-------------------------------------}
function TChatView.FindLineItemAtPos(X,Y: Integer): Integer;
...
for n := 1 to DrawContainers.Count - 1 do
  ...
  if (Y <= dli.pDrawLineInfo.BaseLine) and
     (Y >= dli.pDrawLineInfo.BaseLine - dli.pDrawLineInfo.MaxHeight) and
     (X > dli.Left) and (X < dli.Left + dli.Width) then
    begin
    //мышь над контейнером или в междустрочном пространстве
    result := n;
    break;
    end;
{  else
    begin
    //возможно мышь на полях или между правым краем и последним контейнером
    //проверям...
    if (n = GetFirstContainerInLine(n)) and
       (Y <= dli.pDrawLineInfo.BaseLine) and
    ...
    if (n = GetLastContainerInLine(n)) and
       (Y <= dli.pDrawLineInfo.BaseLine) and
       (Y >= dli.pDrawLineInfo.BaseLine - dli.pDrawLineInfo.MaxHeight) then
      begin
...
{------------------------------------------------------------------}
procedure TChatView.SetSelBounds(SelStartX, SelStartY, SelEndX, SelEndY: Integer);
var dli: TDrawContainerInfo;
    OffsWordNumber, styleno, StartCont, EndCont, i, bs, be, h, cont:integer;
    sz:TSize;
begin
...
//+ еще отслеживаем случаи когда выделение идет по одной строке

//а если контейнер за экраном, то он все равно должен быть выделенным
//хм дело в в координатах рамки выделения...
SelStartX := FSelStartX;
SelStartY := FSelStartY;
SelEndX := FSelEndX;
SelEndY := FSelEndY;

cont := FindLineItemAtPos(SelEndX, SelEndY);
//cont := FindItemAtPos(SelEndX, SelEndY);
h := 0;
if cont >= 0 then
  begin
  be := TDrawContainerInfo(DrawContainers.Objects[cont]).pDrawLineInfo.BaseLine;
  if SelEndY < be then h := be - SelEndY;
  end
else
  be := SelEndY;

cont := FindLineItemAtPos(SelStartX, SelStartY);
//cont := FindItemAtPos(SelStartX, SelStartY);
if cont >= 0 then
  begin
  bs := TDrawContainerInfo(DrawContainers.Objects[cont]).pDrawLineInfo.BaseLine;
  end
else
  bs := SelStartY;
...
if (SelStartY > SelEndY) then
  ...
  begin
  if (SelStartX > SelEndX) and (SelEndY < bs) and (SelEndY < bs) then
    begin
    i := SelEndX;
    SelEndX := SelStartX;
    SelStartX := i;
...
StartCont := FindLineItemAtPos(SelStartX, SelStartY);
if StartCont < 0 then exit;
EndCont := FindLineItemAtPos(SelEndX, SelEndY);
if EndCont < 0 then exit;

dli := TDrawContainerInfo(DrawContainers.Objects[StartCont]);
styleno := TContainerInfo(ContStorage.Objects[dli.ContainerNumber]).StyleNo;
if styleno >= 0 then
  begin
  with FStyle.TextStyles[StyleNo] do
  ...
  GetTextExtentExPoint(Canvas.Handle, PChar(DrawContainers.Strings[StartCont]),
                       Length(DrawContainers.Strings[StartCont]),
                       SelStartX - dli.Left,
                       @OffsWordNumber, nil,
                       sz);

   FSelStartContNo := StartCont;
   FSelStartOffsInCont := OffsWordNumber;
...
{===============================================================================}
   if (no = -5)  then // WinControl
     ...
     if (i > FSelStartContNo) and (i < FSelEndContNo) then
        canv.Rectangle(dli.Left - HPos - 1, dli.Bottom^ - VPos -1,
