...
const
  cvVersion     = 'TChatView v0.34 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
//    procedure CMInvalidate(var Message: TMessage); message CM_INVALIDATE;
    ...
    function GetDrawLineInfo(DrawContainer:cardinal): TDrawLineInfo;
...
begin
  ...
  {------------------------------------------------------------------}
function TChatView.GetDrawLineInfo(DrawContainer:cardinal): TDrawLineInfo;
var i : cardinal;
begin
{result := nil;
for i := 0 to DrawLinesInfo.Count - 1 do
  begin
  if TDrawLineInfo(DrawLinesInfo.Objects[i]).
  result :=
  end;}
end;
  {------------------------------------------------------------------}
