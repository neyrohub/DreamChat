...
const
  cvVersion     = 'TChatView v0.53 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
{поразмыслить над объединением их в один класс}
  ...
  TLinkMouseDown = procedure (Sender:TChatView; DrawContainer: TDrawContainerInfo; LinkInfo:TLinkInfo) of object;//обработчик клика на ссылке
...
//    procedure AdjustChildrenCoords;
  ...
  public
    ...
    Function AddLink(LinkType:Integer; OnLinkMouseMove: TLinkMouseMove;
                           OnLinkMouseDown: TLinkMouseDown;
                           LinkStyle: integer; MouseOverLinkStyle: integer;
                           LinkText:String):TLinkInfo;

    Function DeleteLink(Link: TLinkInfo; SetTextStyle:Integer):boolean;//удаляем ссылку из массива ссылок
...
{-------------------------------------}
Function TChatView.DeleteLink(Link: TLinkInfo; SetTextStyle:Integer):boolean;
  ...
  if LinksInfo.Objects[LinkId] <> nil then
    ...
    for n := 0 to ContStorage.Count - 1 do
      ...
      if Link = TContainerInfo(ContStorage.Objects[n]).LinkInfo then
        ...
        if TContainerInfo(ContStorage.Objects[n]).StyleNo >= 0 then
          begin
//FDebugText := IntToStr(GetLinkIDByLinkPointer(TLinkInfo(LinksInfo.Objects[LinkId])));
FDebugText := IntToStr(SetTextStyle);
self.OnDebug(FDebugText, FDebugText);
          TContainerInfo(ContStorage.Objects[n]).StyleNo := SetTextStyle;
          end;
        end;
      end;
    for n := 0 to DrawContainers.Count - 1 do
...
if MouseOverLinkStyle <= FStyle.TextStyles.Count - 1 then
  LinkInfo.MouseOverLinkStyle := FStyle.TextStyles.Items[MouseOverLinkStyle]
else
  LinkInfo.MouseOverLinkStyle := FStyle.TextStyles.Items[0];
if LinkStyle <= FStyle.TextStyles.Count - 1 then
  LinkInfo.LinkStyle := FStyle.TextStyles.Items[LinkStyle]
else
...
begin
  ...
  if (LinkId >= 0) and (LinksInfo.Count > 0) and (LinkId <= LinksInfo.Count - 1) then
    ...
    //info.StyleNo := TLinkInfo(LinksInfo.Objects[LinkId]).LinkStyle;
...
var ItemAtPos{, no, i, offs,ys, cont}: Integer;
...
if (LastLinkMovedAbove > -1) then
  ...
  if (TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkInfo <> nil) then
    ...
    SetStyleOfLinkObject(LastLinkMovedAbove, GetStyleIDByStylePointer(TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkInfo.LinkStyle));
    InvalidateLinkObject(LastLinkMovedAbove);
    LastLinkMovedAbove := -1;
    end;
  end;
...
if ItemAtPos > 0 then
  ...
  if (TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo <> nil) then
    ...
    SetStyleOfLinkObject(ItemAtPos, GetStyleIDByStylePointer(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo.MouseOverLinkStyle));
    end;
  end;
...
if (ItemAtPos > 0) then
  ...
  FDebugText := FDebugText +
                ...
                '  StyleNo = ' + inttostr(TContainerInfo(ContStorage.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).ContainerNumber]).StyleNo) +
  ...
  for i:=0 to Source.ContStorage.Count-1 do begin
    ...
    case li.StyleNo of
      ...
      -6: AddBullet(li.imgNo, TImageList(li.gr), not li.SameAsPrev);
        ...
        begin
          if li.Center then
               AddCenterLine(Source.ContStorage[i], li.StyleNo, nil)//!!!!!!!!!внимание!!!
          else
             if li.SameAsPrev then
                Add(Source.ContStorage[i], li.StyleNo, nil)//!!!!!!!!!внимание!!!
                //тут потом посмотреть -1 временно!!!
             else
                AddFromNewLine(Source.ContStorage[i], li.StyleNo, nil)//!!!!!!!!!внимание!!!
        end;
    end;
  end;
end;
