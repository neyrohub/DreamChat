...
const
  cvVersion     = 'TChatView v0.38 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
//    procedure CMInvalidate(var Message: TMessage); message CM_INVALIDATE;
    ...
    function FindNearItemAtPos(var X, Y: Integer): Integer;//ищет по всему отображаемому массиву (форматированных) объектов
...
{-------------------------------------}
function TChatView.FindNearItemAtPos(var X, Y: Integer): Integer;
...
cont := FindNearItemAtPos(SelEndX, SelEndY);
...
cont := FindNearItemAtPos(SelStartX, SelStartY);
...
if styleno >= 0 then
  ...
  FSelStartPixOffsInCont := SelStartX - dli.Left;
  if (FSelStartPixOffsInCont > dli.Width div 2) then
    FSelStartContNo := StartCont
  else
    FSelStartContNo := StartCont;
  end;
