...
const
  cvVersion     = 'TChatView v0.46 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
//    procedure AddGifAni(s: String;imgNo: Integer; GifAniObject: TGifAni; fromnewline: Boolean);
    procedure AddGifAni(s: String; Gif: TGif; fromnewline: Boolean);
...
procedure TChatView.AddGifAni(s: String; Gif: TGif; fromnewline: Boolean);
var info: TContainerInfo;
    n:cardinal;
...
if createGifAniObject = true then
  ...
  GifAniObject := TGifAni.Create(Gif);
...
while n <= DrawContainers.Count - 1 do
  begin
  dli := TDrawContainerInfo(DrawContainers.Objects[n]).pDrawLineInfo;
  if (dli.BaseLine - TDrawContainerInfo(DrawContainers.Objects[n]).pDrawLineInfo.MaxHeight) > (VScrollPos + self.Canvas.ClipRect.BottomRight.y {* 2}) then
    begin
    result := n - 1;
    break;
    end;
...
{===============================================================================}
   if (no = -8)  then // gifanimate
     begin
//      TGifAni(li.gr).MirrorImagesY[li.imgNo] := dli.Bottom^ - dli.Height - VScrollPos {* VScrollStep};
//      TGifAni(li.gr).MirrorImagesX[li.imgNo] := dli.Left - HScrollPos;
...
//      else
//        begin
//        TGifAni(li.gr).DestCanvas := canvas;
//        TGifAni(li.gr).DrawFrame(li.imgNo);
