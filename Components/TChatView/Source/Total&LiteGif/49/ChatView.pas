...
const
  cvVersion     = 'TChatView v0.49 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
{поразмыслить над объединением их в один класс}
  ...
  {------------------------------------------------------------------}
  TLinkMouseMove = procedure (DrawContainer: TDrawContainerInfo; LinkInfo:TLinkInfo) of object;//обработчик клика на ссылке
  TLinkMouseDown = procedure (DrawContainer: TDrawContainerInfo; LinkInfo:TLinkInfo) of object;//обработчик клика на ссылке
...
//    procedure AdjustChildrenCoords;
    ...
    function GetLastContainerInCurrLine(FromContainerNumber: cardinal): cardinal;
    function GetFirstContainerInCurrLine(FromContainerNumber: cardinal): cardinal;
  ...
  public
    ...
    Function AddLink(LinkType:Integer; OnLinkMouseMove: TLinkMouseMove;
             OnLinkMouseDown: TLinkMouseDown; LinkText:String):integer;//добавляем ссылку в массив ссылок
...
var ItemAtPos, i,no, StyleNo: Integer;
    clickedword: String;
   dli:TDrawContainerInfo;
    li: TContainerInfo;
    r :TRect;
...
if ItemAtPos > 0 then
  ...
  if TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId >= 0 then
    ...
    if Assigned(TempLink.OnLinkMouseDown) then
      TempLink.OnLinkMouseDown(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]), TempLink);
...
if ItemAtPos > 0 then
  ...
  if TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId >= 0 then
    ...
    if Assigned(TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId]).OnLinkMouseMove) then
      TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId]).OnLinkMouseMove(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]), TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId]));
    Cursor := FStyle.JumpCursor;
    ...
    SetStyleOfLinkObject(ItemAtPos, 2);
...
{-------------------------------------}
function TChatView.GetFirstContainerInCurrLine(FromContainerNumber: cardinal): cardinal;
...
{-------------------------------------}
function TChatView.GetLastContainerInCurrLine(FromContainerNumber: cardinal): cardinal;
var n, CurrentLine: cardinal;
//    dli : TDrawLineInfo;
begin
