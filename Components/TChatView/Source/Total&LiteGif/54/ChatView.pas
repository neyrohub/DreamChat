...
const
  cvVersion     = 'TChatView v0.54 by Bajenov Andrey';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
{поразмыслить над объединением их в один класс}
  ...
  private
    ...
    procedure SetStyleOfLinkObject(ContNum: Integer; LinkStyle: TFontInfo);
...
procedure TChatView.SetStyleOfLinkObject(ContNum: Integer; LinkStyle: TFontInfo);
...
if ContNum >= 0 then
  ...
  dli := TDrawContainerInfo(DrawContainers.Objects[ContNum]);
  li := TContainerInfo(ContStorage.Objects[dli.ContainerNumber]);
...
if (LastLinkMovedAbove > -1) then
  ...
  if (TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkInfo <> nil) then
    begin
    //  SetStyleOfLinkObject(LastLinkMovedAbove, TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkId]).LinkStyle);
...
if ItemAtPos > 0 then
  ...
  if (TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo <> nil) then
    ...
    //SetStyleOfLinkObject(ItemAtPos, TLinkInfo(LinksInfo.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkId]).MouseOverLinkStyle);
...
//      MessageBox(0, Pchar(inttostr(PrevBaseLine)), 'PrevBaseLine', mb_ok);
    ...
    begin { text }
      ...
      //метрику шрифта
      //проверяем, что такой номер стиля есть и не удален!
      if StyleNo > FStyle.TextStyles.Count - 1 then StyleNo := 0;
      with FStyle.TextStyles[StyleNo] do
        begin
        Canvas.Font.Style := Style;
        Canvas.Font.Size  := Size;
