unit ChatView;
//короче проблема в том, что при резком сдвиге канваса изображение иногда выводится на
//реальный канвас. Он же в это время является всего лишь небольшой полоской и
//начало координат у него сдвинуто относительно начала окна
{.$DEFINE Debug}
...
const
  cvVersion     = 'TChatView v0.59 by Bajenov Andrey. Copy right 2008(c)';
  cvsBreak      = -1;
  cvsCheckPoint = -2;
  cvsPicture    = -3;
  cvsHotSpot    = -4;
...
type
  ...
  {Объект-контейнер, который содержит инфу о том в какие координаты X,Y
   ...
   быть текстом, GIF, Control и т.д.}
     ...
     LinkInfo: TLinkInfo;//раньше был адрес на объект TLinkInfo, но при сравнении
     //никак не мог найти одинаковых.
...
{поразмыслить над объединением их в один класс}
  ...
  {Объект-контейнер, который содержит инфу о добавленных объектах, это
   может быть текстом, GIF, Control и т.д.}
     TxtStyle: TFontInfo;
     ObjStyle: integer;//если это не текст, то сюда помещаем номер объекта
     SameAsPrev: Boolean;
     Center: Boolean;
     imgNo: Integer; { for cvsJump# used as jump id }
     gr: TPersistent;
     fon: TBitMap;
     //LinkId: integer;//так было раньше. Но если удаляем ссылку из середины LinksInfo
                       //ID всех последующих ссылок смещаются!
     LinkInfo: TLinkInfo;//так было раньше
  end;
  {------------------------------------------------------------------}
  TCPInfo = class
    public
  ...
  {------------------------------------------------------------------}
  TLinkMouseMove = procedure (SenderChatView: TComponent{TChatView}; DrawContainer: TDrawContainerInfo; LinkInfo:TLinkInfo) of object;//обработчик клика на ссылке
  TLinkMouseUp = procedure (Button: TMouseButton; X, Y: Integer; SenderChatView: TComponent{TChatView}; DrawContainer: TDrawContainerInfo; LinkInfo:TLinkInfo) of object;//обработчик клика на ссылке
  ...
  TCVDblClickEvent = procedure  (Sender: TChatView; ClickedWord: String; Style: TFontInfo) of object;
  ...
  {------------------------------------------------------------------}
  TLinkInfo = class
  {Этот объект содержит в себе информацию о типе ссылки, о том как ее отобразить
  и как ее обработать. Будет создан массив ссылок. Каждый отформатированый
  объект вывода DrawContainerInfo, будет иметь указатель на один из объектов
  ссылки или просто nil. Несколько DrawContainerInfo могут ссылаться на один
  и тотже объект ссылку. Например GIF-контейнер и идущий за ним текст-контейнер}
     LinkType: Integer;
     //LinkStyle: integer;//так было когда хранили ID стиля
     //MouseOverLinkStyle: integer;//но теперь нужно хранить указатель, чтобы
     LinkStyle: TFontInfo;//при удалении стиля и смене ID стелей следующих за
     MouseOverLinkStyle: TFontInfo;//удаленным по ссылке все работало!
     OnLinkMouseMove: TLinkMouseMove;
     OnLinkMouseUp: TLinkMouseUp;
     LinkText:String;
     Hint:String;
  end;
  {------------------------------------------------------------------}
  ...
  private
    ...
    FCursorPosX, FCursorPosY, FCursor: integer;//x, y в пикселах, FCursor номер контейнера
    FGifAniObjNo : word;
    //procedure InvalidateJumpRect(ContNum: Integer);
    procedure SetStyleOfLinkObject(DrawContNum: Integer; LinkStyle: TFontInfo);
...
//    procedure WMNCMOUSEMOVE(var Message: TMessage); message WM_NCMOUSEMOVE;
    ...
    function GetStyleIDByStylePointer(FontInfo: TFontInfo): Integer;
    function GetLinkIDByLinkPointer(LinkInfo: TLinkInfo): Integer;
...
//    function GetPrevContainerInThisLine(DrawCont:TDrawContainerInfo): TContainerInfo;
    ...
    function FindNearestItemAtPos(X, Y: Integer): Integer;//ищет по всему отображаемому массиву (форматированных) объектов
...
//    function FindWordAtScreenPos(ScrX,ScrY: Integer): string; доделать!!
    ...
    function FindNearestItemAtScreenPos(ScrX, ScrY: Integer): Integer;//ищет по всему отображаемому массиву (форматированных) объектов
    ...
    function FindClickedWord(var clickedword: String; var Style: TFontInfo): Boolean;
    procedure Paint; override;
    Function GetCanvas():TCanvas;
    Function AddLink(LinkType:Integer; OnLinkMouseMove: TLinkMouseMove;
                     OnLinkMouseUp: TLinkMouseUp;
                     LinkStyle: TFontInfo; MouseOverLinkStyle: TFontInfo;
                     LinkText:String):TLinkInfo;

    Function UpDateLink(Link: TLinkInfo):boolean;
    Function DeleteLink(Link: TLinkInfo; SetTextStyle: TFontInfo):boolean;//удаляем ссылку из массива ссылок
    procedure AddFromNewLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
    procedure Add(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
    procedure AddCenterLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
    procedure AddText(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
    procedure AddTextFromNewLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
    procedure AddBreak;
...
//    procedure AddControl(ctrl: TWinControl; center: Boolean);
    procedure AddWinControl(ctrl: TWinControl; center: Boolean; LinkInfo: TLinkInfo);
//    procedure AddGifAni(s: String;imgNo: Integer; GifAniObject: TGifAni; fromnewline: Boolean);
    procedure AddGifAni(s: String; Gif: TGif; fromnewline: Boolean; LinkInfo: TLinkInfo);
...
//для чего это нужно? для принтера наверное...
end;
{==================================================================}
...
begin
  ...
  GifAniStorage         := TStringList.Create;
...
//  FCursorSelection      := False;
  FCursorSelection      := True;//включаем курсор, разрешен только одиночный выбор контейнера
  FCursorPosX           := 0;//текущая позиция курсора абсолютные координаты X
  FCursorPosY           := 0;//текущая позиция курсора абсолютные координаты Y
  FCursor               := 0;//текущая позиция курсора (номер отформатированного контейнера)
  ScrollTimer           := nil;
  TimerScrollStepY      := 10;
  ScrollToY             := -1;
  FVScrollBound         := 20;
  ...
  //это стиль для самого первого "пустого" контейнера
  StartFontInfo := TFontInfo.Create(nil);
  StartFontInfo.Style   := [];
  StartFontInfo.Size    := 10;
  StartFontInfo.FontName    := 'Arial';
  StartFontInfo.CharSet := DEFAULT_CHARSET;

  AddFromNewLine('', 0, nil);
...
{  for n := 0 to DrawContainers.Count - 1 do
  ...
  if ContStorage = nil then exit;
  Clear;
//  GifFrame.Free;


  //т.к. ф-ция форматирования работает правильно, только если в хранилище контейнеров
  //есть как минимум 1 объект, то ф-ция Clear, после удаления всего содержимого создает
  //в хранилище объектов ContStorage 1 объект строку (пустую, без текста).
  //соответственно после вызова Clear; в ContStorage у нас есть объект!!!!
  //вот его мы сейчас и убъем
  GifAniStorage.Free;

  ContStorage.BeginUpdate;
  TContainerInfo(ContStorage.objects[ContStorage.Count - 1]).Free;
  ContStorage.objects[ContStorage.Count - 1] := nil;
  ContStorage.Clear;
  ContStorage.EndUpdate;

  FreeAndNil(FBackBitmap);
  FreeAndNil(BufferVirtCanv);
  ...
  //уничтожаем объекты-типы ссылок
  ...
  FreeAndNil(LinksInfo);
  FreeAndNil(DrawLinesInfo);
  FreeAndNil(DrawContainers);
  FreeAndNil(checkpoints);
  FreeAndNil(jumps);
  if not ShareContents then FreeAndNil(ContStorage);
  if StartFontInfo <> nil then FreeAndNil(StartFontInfo);
  inherited Destroy;
end;
{-------------------------------------}
...
{-------------------------------------}
procedure TChatView.WMGetMinMaxInfo(var Message: TWMGetMinMaxInfo);
begin
inherited;
end;
{-------------------------------------}
...
begin
  ...
  FSelStartOffsInCont := 0;
  FSelEndOffsInCont := 0;
  if TContainerInfo(ContStorage.Objects[TDrawContainerInfo(DrawContainers.Objects[FSelEndContNo]).ContainerNumber]).TxtStyle.Index >= 0 then
...
begin
  ...
  if not ShareContents then
    begin
    ContStorage.BeginUpdate;
    //для Гиф анимации у меня отдельный массив. Просто убиваем там все смайлы.
    if GifAniStorage.Count > 0 then
      begin
      for i := 0 to GifAniStorage.Count - 1 do
        begin
        TGifAni(GifAniStorage.objects[i]).free;
        end;
      GifAniStorage.Clear;
      end;
    ...
    for i := 0 to ContStorage.Count - 1 do
      begin
      if TContainerInfo(ContStorage.objects[i]).ObjStyle = -3 then { image}
        begin
        TContainerInfo(ContStorage.objects[i]).gr := nil;
        end;
      if TContainerInfo(ContStorage.objects[i]).ObjStyle = -5 then {wincontrol}
        begin
//        RemoveControl(TControl(TContainerInfo(ContStorage.objects[i]).gr));
//        TContainerInfo(ContStorage.objects[i]).gr.Free;
//        TContainerInfo(ContStorage.objects[i]).gr := nil;
        end;
      if TContainerInfo(ContStorage.objects[i]).ObjStyle = -8 then {GifAni}
        begin
        //Может возникнуть ошибка, если в UNIT1 GIFImage1.free;
        //сделали до ChatView1.clear;

        //Тут ничего дополнительно делать для уничтожения объектов анимации не надо!
        //Все что нужно уже сделано в цикле выше. Массив объектов анимации разрушен
        //сейчас в TGifAni(TContainerInfo(ContStorage.objects[i]).gr битые ссылки
        //на уничтоженные объекты. Вот и не надо их трогать! Просто разрушаем
        //контейнер, содержащий инфу о форматировании.
  ...
  //уничтожаем объекты-типы ссылок
  //тут не надо этого делать!!!
  {if LinksInfo.Count > 0 then
    begin
    for i := 0 to LinksInfo.Count - 1 do
      begin
      TLinkInfo(LinksInfo.Objects[i]).Free;
      end;
    end;}
...
{-------------------------------------}
Function TChatView.GetLinkIDByLinkPointer(LinkInfo: TLinkInfo): Integer;
var i: integer;
begin
result := -1;
if LinksInfo = nil then exit;
for i := 0 to LinksInfo.count - 1 do
  begin
  if TLinkInfo(LinksInfo.Objects[i]) = LinkInfo then
    begin
    result := i;
    break;
    end;
...
{-------------------------------------}
Function TChatView.GetStyleIDByStylePointer(FontInfo: TFontInfo): Integer;
var i: integer;
begin
result := 0;
for i := 0 to FStyle.TextStyles.Count - 1 do
  begin
  if FStyle.TextStyles.Items[i] = FontInfo then
    begin
    result := i;
    break;
    end;
...
result := nil;
if (FStyle = nil) or ((LinkStyle  = nil) and (MouseOverLinkStyle = nil)) then exit;
LinkInfo := TLinkInfo.Create;
//LinkInfo.MouseOverLinkStyle := MouseOverLinkStyle;
//LinkInfo.LinkStyle := LinkStyle;
if LinkStyle <> nil then
  LinkInfo.LinkStyle := LinkStyle
else
  LinkInfo.LinkStyle := FStyle.TextStyles.Items[0];

if MouseOverLinkStyle <> nil then
  LinkInfo.MouseOverLinkStyle := MouseOverLinkStyle
else
  LinkInfo.MouseOverLinkStyle := FStyle.TextStyles.Items[0];

LinkInfo.LinkType := LinkType;
LinkInfo.LinkText := LinkText;
LinkInfo.OnLinkMouseMove := OnLinkMouseMove;
LinkInfo.OnLinkMouseUp := OnLinkMouseUp;
//result:= LinksInfo.AddObject('', LinkInfo);
i := LinksInfo.AddObject('', LinkInfo);
result := TLinkInfo(LinksInfo.Objects[i]);
end;
{-------------------------------------}
Function TChatView.UpDateLink(Link: TLinkInfo):boolean;
var LinkId, i: integer;
begin
result := false;
for i := 0 to DrawContainers.Count - 1 do
  begin
  if (TDrawContainerInfo(DrawContainers.Objects[i]).LinkInfo = Link) then
    begin
    SetStyleOfLinkObject(i, Link.LinkStyle);
    InvalidateLinkObject(i);
    end;
  result := true;
  end;
end;
{-------------------------------------}
Function TChatView.DeleteLink(Link: TLinkInfo; SetTextStyle: TFontInfo):boolean;
var LinkId, n: integer;
begin
//мы должны не только удалить ссылку с таким id из массива ссылок, но и
//пробежаться по всем контейнерам и обнулить указатели на эту ссылку
//а вообще может не юзать указатели? а в контейнерах хранить id?
//подумать над этим на досуге...
//после удаления ссылки назначаем дефолтный Текстовый стиль!
result := false;
if (Link = nil) or (SetTextStyle = nil) then exit;
LinkId := GetLinkIDByLinkPointer(Link);
if LinkId < 0 then exit;
if (LinksInfo.Count > 0) and (LinkId >= 0) and (LinkId <= LinksInfo.Count - 1) then
  begin
  if LinksInfo.Objects[LinkId] <> nil then
    begin
    for n := 0 to ContStorage.Count - 1 do
      begin
      if Link = TContainerInfo(ContStorage.Objects[n]).LinkInfo then
        begin
        TContainerInfo(ContStorage.Objects[n]).LinkInfo := nil;
        //только для текстово контейнера!
        if TContainerInfo(ContStorage.Objects[n]).TxtStyle <> nil then
          begin
          {$IFDEF Debug}
          FDebugText := IntToStr(SetTextStyle.Index);
          //self.OnDebug(FDebugText, FDebugText);
          {$ENDIF}
          TContainerInfo(ContStorage.Objects[n]).TxtStyle := SetTextStyle;
          end;
        end;
      end;
    for n := 0 to DrawContainers.Count - 1 do
      begin
      if Link = TDrawContainerinfo(DrawContainers.Objects[n]).LinkInfo then
        TDrawContainerinfo(DrawContainers.Objects[n]).LinkInfo := nil;
      end;
...
{-----------------}
procedure TChatView.AddFromNewLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
var info: TContainerInfo;
    LinkId: Integer;
begin
//  if (StyleNo > FStyle.TextStyles.Count - 1) then exit;
  info := TContainerInfo.Create;
  if FStyle = nil then
    info.TxtStyle := StartFontInfo
  else
    info.TxtStyle := FStyle.TextStyles.Items[StyleNo];
  info.ObjStyle := 0;
  info.SameAsPrev := False;
  info.Center := False;
  LinkId := GetLinkIDByLinkPointer(LinkInfo);
  if (LinkId >= 0) and (LinksInfo.Count > 0) and (LinkId <= LinksInfo.Count - 1) then
    begin
    info.LinkInfo := LinkInfo;
    //info.StyleNo := GetStyleIDByStylePointer(TLinkInfo(LinksInfo.Objects[LinkId]).LinkStyle);
    info.TxtStyle := TLinkInfo(LinksInfo.Objects[LinkId]).LinkStyle;
    end
  else
    info.LinkInfo := nil;
...
{-------------------------------------}
procedure TChatView.Add(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
var info: TContainerInfo;
    LinkId: Integer;
begin
//если добавляем текст без ссылки, то передаем LinkId := -1!!!
//Добавляет строку текста как есть, т.е. #10#13
//будет отображаться квадратиком
  if (FStyle = nil) or (StyleNo > FStyle.TextStyles.Count - 1) then exit;
  info := TContainerInfo.Create;
  info.TxtStyle := FStyle.TextStyles.Items[StyleNo];
  info.ObjStyle := 0;
...
{-------------------------------------}
procedure TChatView.AddText(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
var p: Integer;
begin
...
//то произойдет перенос текста на следующую строку
   ...
   if p = 0 then begin
     if s <> '' then Add(s, StyleNo, LinkInfo);
     exit;
   end;
   Add(Copy(s, 1, p - 1), StyleNo, LinkInfo);
   Delete(s, 1, p + 1);
   while s<>'' do begin
     p := Pos(chr(13) + chr(10), s);
     if p = 0 then begin
        AddFromNewLine(s, StyleNo, LinkInfo);
        break;
     end;
     AddFromNewLine(Copy(s, 1, p - 1), StyleNo, LinkInfo);
     Delete(s, 1, p + 1);
   end;
end;
{-------------------------------------}
procedure TChatView.AddTextFromNewLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
var p: Integer;
begin
   s:=AdjustLineBreaks(s);
   p := Pos(chr(13)+chr(10),s);
...
{-------------------------------------}
procedure TChatView.AddCenterLine(s: String; StyleNo: Integer; LinkInfo: TLinkInfo);
var info: TContainerInfo;
    LinkId: Integer;
begin
  if StyleNo > FStyle.TextStyles.Count - 1 then exit;
...
procedure TChatView.AddGifAni(s: String; Gif: TGif; fromnewline: Boolean; LinkInfo: TLinkInfo);
var info: TContainerInfo;
    n:cardinal;
    createGifAniObject:boolean;
    GifAniObject: TGifAni;
    LinkId, imgNo: Integer;
begin
//делаем следующим образом: Если в хранилище GifAniStorage нет еще такого смайла
//создаем его и добавляем, если есть, используем ссылку на уже существующий
//при любом раскладе при создании TGifAni прибавляем счетчик ссылок AddMirrorImages
//на количество изображений этого вида смайла (объект при этом не дублируется)
imgNo := 0;
createGifAniObject := true;
if (ContStorage.count > 0) and (GifAniStorage.count > 0) then
  for n := 0 to GifAniStorage.count - 1 do
    begin
    GifAniObject := TGifAni(GifAniStorage.objects[n]);
    if Gif = GifAniObject.GifImage then
      begin
      //такой GIF уже есть!
      GifAniObject := TGifAni(GifAniStorage.objects[n]);
...
if createGifAniObject = true then
  begin
  //такой GIF еще не добавлялся
  GifAniObject := TGifAni.Create(Gif, self.Style.Color);
  GifAniStorage.AddObject('', GifAniObject);
...
procedure TChatView.AddWinControl(ctrl: TWinControl; center: Boolean; LinkInfo: TLinkInfo); { do not free ctrl! }
var info: TContainerInfo;
    LinkId: Integer;
begin
...
dli : TDrawLineInfo;
begin
//если даже самый последний контейнер помещается в VScrollPos + Y размер канваса, то
...
procedure TChatView.SetStyleOfLinkObject(DrawContNum: Integer; LinkStyle: TFontInfo);
var dli: TDrawContainerInfo;
    n: Integer;
    link: TLinkInfo;
    li: TContainerInfo;
begin
//если наводим на ссылку мышью, то все объекты с этой ссылкой меняют свой стиль!
//изменяем стиль отрисовки объекта (теперь он выделен)
if DrawContNum >= 0 then
  begin
  //меняем стиль ссылки под курсором мыши
  dli := TDrawContainerInfo(DrawContainers.Objects[DrawContNum]);
  li := TContainerInfo(ContStorage.Objects[dli.ContainerNumber]);
  if li.ObjStyle >= 0 then
    TContainerInfo(ContStorage.Objects[dli.ContainerNumber]).TxtStyle := LinkStyle;
  //пройдемся по всем ссылкам
{  link := TDrawContainerInfo(DrawContainers.Objects[ContNum]).LinkInfo;
...
//    li: TContainerInfo;
    ...
    link: TLinkInfo;
begin
//эта процедура отрисовывает часть канваса, которую занимает некий объект
//она полностью заменяет процедуру InvalidateJumpRect и щас используется
//для отрисовки объектов-ссылки, являющихся одной ссылкой,
//при прохождении над ним мыши
if Style.FullRedraw then
  Invalidate
else
  begin
  if ContNum >= 0 then
    ...
    for n := FirstVisibleContainer to LastVisibleContainer do
      ...
      if link = TDrawContainerInfo(DrawContainers.Objects[n]).LinkInfo then
        begin
        dli := TDrawContainerInfo(DrawContainers.Objects[n]);
        if TContainerInfo(ContStorage.Objects[dli.ContainerNumber]).ObjStyle >= 0 then
          begin
...
if Button = mbRight then
  ...
  exit;
  end;//if Button <> mbLeft then exit;
...
var ItemAtPos, i{, no, i, offs,ys, cont}: Integer;
...
if (LastLinkMovedAbove > -1) then
  begin
  if (TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkInfo <> nil) then
    begin
    SetStyleOfLinkObject(LastLinkMovedAbove, TDrawContainerInfo(DrawContainers.Objects[LastLinkMovedAbove]).LinkInfo.LinkStyle);
    InvalidateLinkObject(LastLinkMovedAbove);
    LastLinkMovedAbove := -1;
    end;
  end;
...
if ItemAtPos > 0 then
  begin
  if (TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo <> nil) then
    begin
    if Assigned(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo.OnLinkMouseMove) then
      TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo.OnLinkMouseMove(Self, TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]), TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo);
    Cursor := FStyle.JumpCursor;
    FDebugText := FDebugText + TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo.LinkText + #10#13;
    LastLinkMovedAbove := ItemAtPos;

    SetStyleOfLinkObject(ItemAtPos, TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo.MouseOverLinkStyle);
    end;
  end;
...
if (ItemAtPos > 0) then
  begin
  if TContainerInfo(ContStorage.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).ContainerNumber]).TxtStyle <> nil then
    i := TContainerInfo(ContStorage.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).ContainerNumber]).TxtStyle.Index
  else
    i := TContainerInfo(ContStorage.Objects[TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).ContainerNumber]).ObjStyle;

  FDebugText := FDebugText +
                'MouseMove' + '    LastLinkMovedAbove ='+ inttostr(LastLinkMovedAbove) +
                '  StyleNo = ' + inttostr(i) +
                #10#13 + 'Line =' + inttostr(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).pDrawLineInfo.LineNumber) +
                '   BaseLine =' + inttostr(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).pDrawLineInfo.BaseLine) +
...
if FindNearestItemAtScreenPos(x, y) > 0 then
  begin
  ItemAtPos := FindNearestItemAtScreenPos(x, y);
  FDebugText := FDebugText +
                #10#13 + 'GetFirstContainerInCurrLine = ' + inttostr(GetFirstContainerInCurrLine(ItemAtPos)) +
                '   GetLastContainerInCurrLine = ' + inttostr(GetLastContainerInCurrLine(ItemAtPos)) +
                #10#13 + 'MaxHeight =' + inttostr(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).pDrawLineInfo.MaxHeight) +
                ' Left =' + inttostr(TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).Left) +
...
if ItemAtPos > 0 then
  ...
  if TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo <> nil then
    begin
    TempLink := TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]).LinkInfo;
    if Assigned(TempLink.OnLinkMouseUp) then
      begin
      //если присвоен обработчик клика на ссылке
      if (Button = mbRight) or
        ((ItemAtPos < FSelStartContNo) and (FSelEndContNo > ItemAtPos)) or
        ((ItemAtPos = FSelStartContNo) and (ItemAtPos = FSelEndContNo) and
        (FSelStartOffsInCont = FSelEndOffsInCont)) then
        begin
        //и если этот контейнер не попадает в зону выделения.
        //тогда осуществляем вызов пользовательского обработчика клика.
        TempLink.OnLinkMouseUp(Button, X, Y, Self, TDrawContainerInfo(DrawContainers.Objects[ItemAtPos]), TempLink);
        end;
      end;
      {$IFDEF Debug}
      FDebugText2 := 'MouseUP' + #10#13 + 'ItemAtPos = ' + inttostr(ItemAtPos);
      //self.OnDebug(FDebugText, FDebugText2);
      {$ENDIF}
    end;
  end;
  ...
  for i:=0 to Source.ContStorage.Count - 1 do begin
    ...
    case li.ObjStyle of
      ...
      -6: AddBullet(li.imgNo, TImageList(li.gr), not li.SameAsPrev);
        ...
        begin
          if li.Center then
               AddCenterLine(Source.ContStorage[i], li.ObjStyle, nil)//!!!!!!!!!внимание!!!
          else
             if li.SameAsPrev then
                Add(Source.ContStorage[i], li.ObjStyle, nil)//!!!!!!!!!внимание!!!
                //тут потом посмотреть -1 временно!!!
             else
                AddFromNewLine(Source.ContStorage[i], li.ObjStyle, nil)//!!!!!!!!!внимание!!!
        end;
    end;
  end;
end;
...
//выдает номер первого контейнера в линии. Поиск начинается с FromContainerNumber
...
{-------------------------------------}
{
доделать! тут нужно искать не только в этом, но и в соседних контейнерах,
до тех пор, пока не встретится пробел один из делимитеров начале и в конце

function TChatView.FindWordAtScreenPos(ScrX, ScrY: Integer): string;
var ItemAtPos, i: integer;
begin
result := '';
ItemAtPos := FindItemAtScreenPos(ScrX, ScrY);
if ItemAtPos > 0 then
  begin
  result := DrawContainers.Strings[ItemAtPos];
  end;
result := FindSymbolAtScreenPos(ScrX, ScrY);
end;}
{-------------------------------------}
...
//не только когда на него неводят мышью, но и когда мышь находиться выше контейнера
//но в пределах его |    |
//т.е. в пустотах между строк и на полях эта ф-ция выдает -1
var
...
//FindNearItemAtPos отличается от FindNearestItemAtPos тем, что выдает номер контейнера
//не только когда на него наводят мышью, но и когда мышь находиться в пустотах
...
{if Y >= TDrawContainerInfo(DrawContainers.Objects[DrawContainers.Count - 1]).pDrawLineInfo.BaseLine then
  ...
  for n := FirstInCurrLine to DrawContainers.Count - 1 do
    begin
    //находим под каким контейнером по Х он находится
    dli := TDrawContainerInfo(DrawContainers.Objects[n]);
...
{  for n := 1 to DrawContainers.Count - 1 do
  ...
  //над контейнерами не нашли ищем между и на полях
  for n := 1 to DrawContainers.Count - 1 do
    begin
    dli := TDrawContainerInfo(DrawContainers.Objects[n]);
    if (Y <= dli.pDrawLineInfo.BaseLine) then
      begin
      //если курсор мыши находится выше очередной строки
      if (X <= dli.Left + dli.Width) then
        begin
        //мышь слева перед контейнером
        result := n;
        exit;
        end;
...
for n := 0 to DrawContainers.Count - 1 do
  ...
  {------------------------------------------------------------------}
function TChatView.FindClickedWord(var clickedword: String; var Style: TFontInfo): Boolean;
var no, lno: Integer;
...
begin
  ...
  if no <> -1 then begin
     ...
     style := TContainerInfo(ContStorage.Objects[lno]).TxtStyle;
     if style <> nil then
       begin
       with style do
          begin
...
begin
  ...
  if FindClickedWord(clickedword, Style) then
     FOnCVDblClick(Self, clickedword, Style);
end;
  {------------------------------------------------------------------}
...
style := TContainerInfo(ContStorage.Objects[dli.ContainerNumber]).txtstyle;
if style <> nil then
  begin
  with Style do
    begin
...
var
  ...
  OffsWordNumber, StartCont, EndCont, i, cont: integer;
  Style: TFontInfo;
  sz:TSize;
begin
...
/////      dli := TDrawContainerInfo(DrawContainers.Objects[n]);
      {$IFDEF Debug}
      FDebugText2 := 'Start между' + '  n:=' + inttostr(n);
      self.OnDebug(FDebugText, FDebugText2);
      {$ENDIF}
      FSelStartContNo := n;
      FSelStartOffsInCont := 0;
...
if (FSelEndY < 0) then
  begin
  {$IFDEF Debug}
  FDebugText2 := 'End за верхним краем канваса';
  self.OnDebug(FDebugText, FDebugText2);
  {$ENDIF}
  FSelEndContNo := 0;
  FSelEndOffsInCont := 0;
...
for n := 1 to DrawContainers.Count - 1 do
  begin
  //начинаем перебирать все контейнеры в поисках того, по короторому кликнули
  //или над которым переместилась мышь
  dli := TDrawContainerInfo(DrawContainers.Objects[n]);
    ...
    if (FSelEndX >= dli.Left) and (FSelEndX <= dli.Left + dli.Width) then
      ...
      if Style <> nil then
        ...
        if (FSelStartContNo < FSelEndContNo) then
          begin
          //подводим мышь к смайлу с этой стороны -> []
          {$IFDEF Debug}
          FDebugText2 := 'End БЕЗ ТЕКСТА (EndY > StartY)' +
                         '  n:=' + inttostr(n);
          self.OnDebug(FDebugText, FDebugText2);
          {$ENDIF}
      ...
      if (FSelEndX >= dli.Left + dli.Width) then
        begin
        {$IFDEF Debug}
        FDebugText2 := 'End за последним контейнером этой линии LastInCurrLine = ' +
        IntToStr(LastInCurrLine) + ' n=' + IntToStr(n) + ' DrawContainers.Count - 1=' + IntToStr(DrawContainers.Count - 1);
        self.OnDebug(FDebugText, FDebugText2);
        {$ENDIF}
      ...
      if (n + 1 <= DrawContainers.Count - 1) then
        ...
        if (FSelEndX >= dli.Left + dli.Width) and
          ...
          begin
          {$IFDEF Debug}
          FDebugText2 := 'End между ->' + '  n:=' + inttostr(n);
          self.OnDebug(FDebugText, FDebugText2);
          {$ENDIF}
          FSelEndContNo := n;
        ...
        if (FSelEndX >= dli.Left + dli.Width) and
          ...
          begin
          {$IFDEF Debug}
          FDebugText2 := 'End между <-' + '  n:=' + inttostr(n + 1);
          self.OnDebug(FDebugText, FDebugText2);
          {$ENDIF}
          FSelEndContNo := n + 1;
          FSelEndOffsInCont := 0;
...
if ScrollToYPos >= 0 then
  begin
  //если нам нужно проскролировать до некой конкретной позиции
  ScrollTimer.Enabled := true;
  ScrollToY := ScrollToYPos;
  if VScrollPos > ScrollToY then
  //и эта позиция выше текущей
    VScrollUp := true
  else
    VScrollUp := false;
  end;
...
//далее боремся с заклиниванием таймера при прокрутке
...
//VScrollPos это самая верхняя граница виртуального экрана, отображающего DrawContainers
//а нам надо еще учесть высоту канваса, ведь последняя BaseLine находиться в самом низу
...
if FCursor < 0 then
  ...
  if FCursor < 0 then MessageBox(0, PChar('FindNearItemAtScreenPos вернул FCursor < 0!!!'), PChar('Сейчас все рухнет!') ,mb_ok);
  end;
...
for i := FSelStartContNo to FSelEndContNo do
  ...
  if Style <> nil then
    ...
    if (FSelStartContNo = FSelEndContNo) then
      begin
      //если выделение в пределах одного контейнера
...
//            TempCursor := GetContainerAtXInLine(FCursorPosX, PrevLine);
            ...
            if TempCursor >= 0 then
              ...
              begin
              //если выше не смог найти контейнер. Это могло быть, если
              //1.его там и нет, т.е. это самая верхняя строка
              //2.а может просто нижняя строка это один символ посередине и просто
              //по Х прицел на нее не попадает
              if FCursor - 1 > 1 then
                begin
                dec(FCursor);
...
{          TempCursor := GetContainerAtXInLine(FCursorPosX, NextLine);}
          ...
          if TempCursor >= 0 then
            ...
            begin
            //если ниже не смог найти контейнер. Это могло быть, если
            //1.его там и нет, т.е. это последняя строка
            //2.а может просто нижняя строка это один символ посередине и просто
            //по Х прицел на нее не попадает
...
{          FDebugText2 := 'NextLine= ' + inttostr(NextLine) + ' FCursor=' + inttostr(FCursor)+
        ...
        VK_LEFT:
          ...
          if FCursor > 1 then
            begin
            //если слева еще есть контейнеры
            dec(FCursor);
            FCursorPosX := TDrawContainerInfo(DrawContainers.Objects[FCursor]).Left +
                           TDrawContainerInfo(DrawContainers.Objects[FCursor]).width div 2;
            FCursorPosY := TDrawContainerInfo(DrawContainers.Objects[FCursor]).pDrawLineInfo.BaseLine -
...
{            FDebugText2 := 'NextLine= ' + inttostr(NextLine) + ' FCursor=' + inttostr(FCursor)+
        ...
        VK_RIGHT:
          ...
          if FCursor < DrawContainers.Count - 1 then
            begin
            //если справа еще есть контейнеры
            inc(FCursor);
            FCursorPosX := TDrawContainerInfo(DrawContainers.Objects[FCursor]).Left +
                           TDrawContainerInfo(DrawContainers.Objects[FCursor]).width div 2;
            FCursorPosY := TDrawContainerInfo(DrawContainers.Objects[FCursor]).pDrawLineInfo.BaseLine -
...
for n := 0 to DrawContainers.Count - 1 do
  ...
  if li.ObjStyle = -8 then
    ...
    //найти его номер в MirrorImagesY[n] массве родителя
      //вычитаем начальные координаты куска канваса для отрисовки, т.к. мы их
      //прибавляля в ф-ции Paint.
...
{определяем состояние TChatView компонента}
 if (not (csDesigning in ComponentState)) and (not Assigned(FStyle)) then
    begin
    Canvas.Brush.Color := clWhite;
    Canvas.FillRect(Canvas.ClipRect);
    Canvas.TextOut(ClientRect.Left+1, ClientRect.Top + 1, 'Error: Chatview1.Style is not assigned');
    Canvas.TextOut(ClientRect.Left+1, ClientRect.Top + 15, 'Please add component TCVStyle on the form and choose it Chatview1.Style := CVStyle1');
    Canvas.TextOut(ClientRect.Left+1, ClientRect.Top + 30, 'Or create Style1 := TCVStyle.Create at run time and assing it Chatview1.Style := CVStyle1');
 ...
 if (csDesigning in ComponentState) then
   begin
    ...
    Canvas.TextOut(ClientRect.Left + 1, ClientRect.Top + 15,
                  'You can FREE use this component only with FREEWARE programs!');
    Canvas.TextOut(ClientRect.Left + 1, ClientRect.Top + 31,
                  'Otherwise you should buy it!');
...
//доделать оптимальную отрисовку!!!!!!!!
 ...
 //т.к. Canvas.ClipRect.Top нам возвращает только небольшой участок канваса,
 //который необходтмо отрисовать (например шириной в один пиксел, если мы провели
 //передвинули другое окно поверх нашего на один пиксел вниз)
 //необходимо пересчитать начало координат этого куска канваса
 yshift := VScrollPos + Canvas.ClipRect.Top;
 xshift := HScrollPos + Canvas.ClipRect.Left;
...
//   if dli.top^ > r.Bottom then lastline := True;
   ...
   //проверяем, что такой номер стиля есть и не удален!
   //if no >= FStyle.TextStyles.Count - 1 then No := 0;
   if TxtStyle <> nil then
     begin // text
     canv.Font.Style := TxtStyle.Style;
     canv.Font.Size := TxtStyle.Size;
     canv.Font.Name := TxtStyle.FontName;
     canv.Font.CharSet := TxtStyle.CharSet;
     if not ((TxtStyle.Index in [cvsJump1, cvsJump2]) and DrawHover and
        (LastJumpMovedAbove<>-1) and
        (li.ImgNo = LastJumpMovedAbove)) then
       begin
       textcolor := TxtStyle.Color;
       hovernow := False;
       end
     else
       begin
...
//        - Canvas.ClipRect.Left, - исправляем координаты при перекрытии окном сверху
//        - Canvas.ClipRect.Top,
...
{===============================================================================}
   if (li.ObjStyle = -8)  then // gifanimate
     begin
     ...
     //тут перерисовывать только при скроллинге!!!!!
...
{===============================================================================}
   if (li.ObjStyle = -7)  then // gif }
     begin
     end;
{===============================================================================}
   if (li.ObjStyle = -5)  then // WinControl
     begin
     //рисуем рамку при выделении
     if (i >= FSelStartContNo) and (i <= FSelEndContNo) then
...
{===============================================================================}
   if (li.ObjStyle = -4) or (li.ObjStyle = -6)  then
...
//     TImageList(li.gr).Draw(canv, dli.Left-xshift, dli.top^ -yshift, li.imgNo);
     ...
     TImageList(li.gr).DrawingStyle := TDrawingStyle(dsNormal);
     continue;
   end;
{===============================================================================}
   if (li.ObjStyle = -3)  then
     begin // graphics
...
//   if depth = 0 then StoreSelBounds(StartNo, EndNo, StartOffs, EndOffs);
   ...
   if not (OnlyResized and (self.TextWidth = OldTextWidth)) then
     begin
     if (OnlyTail = true) and (DrawContainers.Count > 0) then
       begin
...
{++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++}
     for i := StartContainer to ContStorage.Count - 1 do
       begin
       TxtStyle := TContainerInfo(ContStorage.Objects[i]).TxtStyle;
       if TxtStyle <> nil then
         StyleNo := TxtStyle.Index
       else
         StyleNo := TContainerInfo(ContStorage.Objects[i]).ObjStyle;
...
{    if DrawLinesInfo.Count > 0 then
  ...
  case TContainerInfo(ContStorage.Objects[ContNum]).ObjStyle of
   -8:  { GifAni}
     begin
       ...
       info.LinkInfo := TContainerInfo(ContStorage.Objects[ContNum]).LinkInfo;
       info.Width  := ctrlw;
...
//      MessageBox(0, Pchar(inttostr(PrevBaseLine)), 'PrevBaseLine', mb_ok);
    ...
    begin { text }
      ...
      //StyleNo := TContainerInfo(ContStorage.Objects[ContNum]).ObjStyle;
      if TContainerInfo(ContStorage.Objects[ContNum]).TxtStyle <> nil then
        StyleNo := TContainerInfo(ContStorage.Objects[ContNum]).TxtStyle.Index;
      //настраиваем канвас на нужный стиль, чтобы правильно получить
      //метрику шрифта
      //проверяем, что такой номер стиля есть и не удален!
      if (StyleNo > FStyle.TextStyles.Count - 1) or (StyleNo <= 0) then StyleNo := 0;
      with FStyle.TextStyles[StyleNo] do
        begin
        Canvas.Font.Style := Style;
        Canvas.Font.Size  := Size;
...
//                    'DrawLineInfo.LineNumber   baseline', mb_ok);
        ...
        //если max меньше, чем длина текста (т.е. текст НЕ ПОМЕЩАЕТСЯ до конца канваса)
          ...
          if strSpacePos <> nil then
            ...
            if x > sad.LeftMargin{0} then max := 0;
