...
var
  CryptoKey                                                          :string;
  CryptoKeyLen                                                       :word;
  InfoForExe, LocalComputerName, LocalLoginName, LocalIpAddres       :string;
...
begin
  ...
  MessageManager.DefaultKey := CryptoKey;
  try
...
var
    ...
    Command, SDebug, HeaderOfProtocol, sNetBiosNameOfRemoteComputer, stemp{, scrypto} :string;
    crypted_out, buffer_out, Full_buffer :array[0..1499] of Char;
    i: integer;
    ppPersonalCrypto: pPersonalCrypto;
{$IFDEF USELOG4D}
    logger: TLogLogger;
{$ENDIF USELOG4D}
begin
...
try
  ...
  try
    if (QueueOfMessages.Count > 0) and (ClientSocket <> nil) and
      ...
      begin
      sNetBiosNameOfRemoteComputer := QueueOfRemoteComputersNames.Strings[0];
      QueueOfRemoteComputersNames.Delete(0);

      stemp := QueueOfMessages.Strings[0];
      QueueOfMessages.Delete(0);

      //находим секретный ключ для данного корреспондента
      DCP_rc41.Init(CryptoKey[1], length(CryptoKey) * 8, nil);
{Доделать персональное шифрование и раскомментарить код ниже
      i := 0;
      ...
      if i >= 0 then
        ...
        ppPersonalCrypto := pPersonalCrypto(CryptoKeyForRemoteComputers.Objects[i]);
        DCP_rc41.Init(ppPersonalCrypto.PersonalSecretKey[0], ppPersonalCrypto.LenPersonalSecretKey * 8, nil);
        end
      else
        begin
        //устанавливаем секретный ключ шифрования по умолчанию
        DCP_rc41.Init(key[1], length(key) * 8, nil);
      ...
      //if  then begin
        ...
        HeaderOfProtocol := LocalComputerName + #00 + 'FORWARD' + #00 + sNetBiosNameOfRemoteComputer + #00;
        SDebug := '[' + LocalComputerName + '][$00][' + 'FORWARD' + '][$00][' + sNetBiosNameOfRemoteComputer + '][$00]';
...
begin
  ...
  CryptoKey := ChatConfig.ReadString('Crypto', 'Key', 'tahci');
  CryptoKeyLen := Length(CryptoKey);
...
//вот тут надо передавать ключ в дешифровщик...
//однако он приходит к нам только при первом коннекте
//возможно проще сделать пока его получение при инициализации?
    MessageManager := TProtocolMessageManager.Create(CryptoKey);
    QueueOfRemoteComputersNames := TStringList.Create;
    CryptoKeyForRemoteComputers := TStringList.Create;
...
function SetNewCryptoKey(pSecretKey: Pointer; SizeSecretKey: word):PChar;
begin
  setlength(CryptoKey, SizeSecretKey);
  CopyMemory(@CryptoKey[1], pSecretKey, SizeSecretKey);
  CryptoKeyLen := SizeSecretKey;
  result := '';
end;
...
//MessageBox(0, PChar('QueueOfMessages free'), PChar(inttostr(0)) ,mb_ok);
    while QueueOfRemoteComputersNames.Count > 0 do
      begin
      QueueOfRemoteComputersNames.Delete(0);
      end;
    QueueOfRemoteComputersNames.Free;

    while CryptoKeyForRemoteComputers.Count > 0 do
      begin
      if (CryptoKeyForRemoteComputers.Objects[0] <> nil) then
        begin
        FreeMem(pPersonalCrypto(CryptoKeyForRemoteComputers.Objects[0]));
        CryptoKeyForRemoteComputers.Delete(0);
        end;
      end;
    CryptoKeyForRemoteComputers.Free;
...
function InternalSendCommDisconnect(pProtoName, pNameOfLocalComputer,
                                    pNetbiosNameOfRemoteComputer, pLineName:PChar):Pchar;
...
{Доделать персональное шифрование и раскомментарить код ниже
  //находим секретный ключ для данного корреспондента
  i := 0;
  if (CryptoKeyForRemoteComputers.Count > 0) and
     (CryptoKeyForRemoteComputers.IndexOf(sNetbiosNameOfRemoteComputer) >= 0) then
    begin
    i := CryptoKeyForRemoteComputers.IndexOf(sNetbiosNameOfRemoteComputer);
    //устанавливаем персональный секретный ключ шифрования
    ppPersonalCrypto := pPersonalCrypto(CryptoKeyForRemoteComputers.Objects[i]);
    ...
    begin
    //добавляем в список персональный ключ
    GetMem(ppPersonalCrypto, SizeOf(TPersonalCrypto));
    ZeroMemory(ppPersonalCrypto, SizeOf(TPersonalCrypto));
    CopyMemory(@(ppPersonalCrypto.PersonalSecretKey), pSecretKey, SizeSecretKey);
    ppPersonalCrypto.LenPersonalSecretKey := SizeSecretKey;

    CryptoKeyForRemoteComputers.AddObject(sNetbiosNameOfRemoteComputer, pointer(ppPersonalCrypto));
    DCP_rc41.Init(ppPersonalCrypto.PersonalSecretKey[0], ppPersonalCrypto.LenPersonalSecretKey * 8, nil);
    end;
}
  //шифруем строку, получаем строку HEX кодов
...
function SendCommDisconnect(pProtoName, pNameOfLocalComputer,
                            pNetbiosNameOfRemoteComputer, pLineName:PChar):Pchar;
{$IFDEF USELOG4D}
var
  logger: TlogLogger;
...
begin
  try
    CriticalSection.Enter;
    try
      Result := InternalSendCommDisconnect(pProtoName,
                                           pNameOfLocalComputer, pNetbiosNameOfRemoteComputer, pLineName);
    except
      on E: Exception do begin
        Result := '';
{$IFDEF USELOG4D}
...
if DCP_rc41 <> nil then begin
  //выделяем память под персональные ключи шифрования с удаленным корреспондертом
  GetMem(ppPersonalCrypto, SizeOf(TPersonalCrypto));
  ZeroMemory(ppPersonalCrypto, SizeOf(TPersonalCrypto));
  CopyMemory(@(ppPersonalCrypto.PersonalSecretKey), @CryptoKey[1], CryptoKeyLen);
  ppPersonalCrypto.LenPersonalSecretKey := CryptoKeyLen;
...
{доделать и раскомментарить код ниже
  //проверяем хранится ли для этого корреспондента ключ в списке секретных ключей
  if (CryptoKeyForRemoteComputers.Count > 0) and
     (CryptoKeyForRemoteComputers.IndexOf(sNetbiosNameOfRemoteComputer) >= 0) then
    begin
    //такой ключ был в списке - удаляем его, т.к. новый может с ним не совпадать
    i := CryptoKeyForRemoteComputers.IndexOf(sNetbiosNameOfRemoteComputer);
    FreeMem(pPersonalCrypto(CryptoKeyForRemoteComputers.Objects[i]));
    CryptoKeyForRemoteComputers.Delete(i);
    end;
  //добавляем новый ключ в список
  CryptoKeyForRemoteComputers.AddObject(sNetbiosNameOfRemoteComputer, pointer(ppPersonalCrypto));
}
  //добавляем сообщение в очередь исходящих сообщений
  QueueOfMessages.Add(stemp);
  QueueOfRemoteComputersNames.Add(sNetbiosNameOfRemoteComputer);
...
begin
  try
    CriticalSection.Enter;
    try
      Result := InternalSendCommConnect(
                           pProtoName, pLocalNickName, pNetbiosNameOfRemoteComputer,
                           pLineName,pNameOfRemoteComputer,
...
function InternalSendCommText(pProtoName, pNetbiosNameOfRemoteComputer, pNickNameOfRemoteComputer:PChar;
                              pMessageText:PChar; ChatLine:Pchar;Increment:integer):Pchar;
...
function SendCommText(pProtoName, pNetbiosNameOfRemoteComputer, pNickNameOfRemoteComputer:PChar;
                      pMessageText:PChar; ChatLine:Pchar;Increment:integer):Pchar;
{$IFDEF USELOG4D}
var
  logger: TlogLogger;
...
function InternalSendCommStatus(pProtoName, pNetbiosNameOfRemoteComputer:PChar;
                                LocalUserStatus:cardinal; StatusMessage:Pchar):Pchar;
var
...
exports
  ...
  SetNewCryptoKey index 22 name 'SetNewCryptoKey';
