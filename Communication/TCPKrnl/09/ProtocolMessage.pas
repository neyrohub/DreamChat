...
uses
  SysUtils, DCPcrypt2, DCPrc4, DreamChatExceptions, log4d;
...
constructor TProtocolMessage.Create(Key: string);
begin
  FPacketSize := 0;
...
begin
  ...
  if IChatPacketSize = 0
    then raise EConvertError.Create('Incorrect data in message buffer received. Packet size is zero.');
  ...
  if (IChatPacketSize < LenOfFieldCommand + 1)
    then raise EConvertError.Create('Incorrect data in message buffer received. Packet size is too small.');
...
//    then raise EConvertError.Create('Incorrect data in message buffer received. Empty message data detected.');
  ...
  if IChatDataLen = 0
    then raise EConvertError.Create('Incorrect data in message buffer received. Message size is zero.');
