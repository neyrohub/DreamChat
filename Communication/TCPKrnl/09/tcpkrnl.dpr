...
var
  ...
  len, n: cardinal;
  MyAdrr: in_addr;
  res, IP, firstPart, Lan, Internet: string;
...
{$ENDIF}
  ...
  //  RunCallBackFunction(PChar('[' + inttostr(n) + '] ' + ip), 0);
    firstPart := copy(IP, 1, pos('.', IP) - 1);
  //  if ip = '127' then нашли Loopback интерфейс;
    if (firstPart <> '127') then
    begin
      if (firstPart = '10') or (firstPart = '172') or (firstPart = '192')
        then Lan := IP //inet_ntoa(MyAdrr)
        else Internet := IP; //inet_ntoa(MyAdrr);
    end;
  end;
  ...
  if ChatConf.ReadString('ConnectionType', 'LocalIP', '127.0.0.1') <> '127.0.0.1'
    then  res := ChatConf.ReadString('ConnectionType', 'LocalIP', '127.0.0.1');
...
BEGIN
  ...
  {  if count > 0 then GetNextIncomingMessageFromMailSlot();
      ...
      on E: Exception do begin
        logger := TLogLogger.GetLogger('dreamchat');
...
begin
 // initialize log4d
 TLogPropertyConfigurator.Configure('tcpkrnl.props');
 logger := TLogLogger.GetLogger('tcpkrnl');
 logger.Error('--------------------------------------------------------');
 logger.Error('------------------------   START   ---------------------');
 logger.Error('--------------------------------------------------------');
