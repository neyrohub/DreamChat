...
//константы храняться в .TAG у каждого пункта меню
//.Command - read only (((
...
type
  ...
  public
    ...
    FParentChatLine:TObject;//из-за невозможности подключить в USES модуль
                            //uChatLine нельзя сделать эту переменную
                            //типа FParentChatLine:TChatLine
    MenuItemId: TMenuItemId;
    FPDNode : PDataNode;
    FVirtualNode: PVirtualNode;
    SpeedButtonNumber: integer;   //сюда записывается номер кнопки, иначе никак
    ...
    procedure AddRxTrayMenu(Sender: TComponent);
  end;
...
uses
  uFormMain, uChatLine, uFormDebug, DreamChatConfig, uPathBuilder, uImageLoader;
...
MenuItemId.Caption := 'Это меню для';
...
if Component is TsChatView then
  ...
  if TChatLine(FParentChatLine) <> FormMain.GetMainLine then
    begin
    MenuItemId := TMenuItemId.Create(Self);
    MenuItemId.Caption := fmInternational.Strings[I_CLOSE];//'Закрыть';
...
if Component is TVirtualStringTree then
  begin
  MenuItemId := TMenuItemId.Create(Self);
  MenuItemId.Caption := fmInternational.Strings[I_REFRESH];//Обновить
...
if (Component is TsMemo) then
  ...
  if TsMemo(Component).Name = 'DebugMemo1' then
    begin
    MenuItemId := TMenuItemId.Create(Self);
    MenuItemId.Caption := fmInternational.Strings[I_SAVELOG];//Сохранить лог
...
Self.Items.Add(DynamicPopupMenu.MenuItemId);
//========= Добавляем строчку меню: Личное сообщение
Self.MenuItemId := TMenuItemId.Create(Self);
...
Self.Items.Add(DynamicPopupMenu.MenuItemId);
//========= Добавляем строчку меню: Личное сообщение всем
Self.MenuItemId := TMenuItemId.Create(Self);
Self.MenuItemId.Caption := fmInternational.Strings[I_PRIVATEMESSAGETOALL] + ' ';//Личное сообщение всем
Self.MenuItemId.Tag := ONMENUTUserSendPrivateMessageToAll;//короче надо в одном обработчике различать разные клики
Self.MenuItemId.OnClick := OnMenuClick;
self.MenuItemId.ImageIndex := Self.Images.AddMasked(TDreamChatImageLoader.GetImage(G_POPUP_MASSMESSAGE).Bitmap[0],
...
if AnsiCompareText(FPDNode.User.ComputerName, TChatLine(FParentChatLine).LocalComputerName) <> 0 then
  begin
  //========= Добавляем строчку меню: Личный чат с
  Self.MenuItemId := TMenuItemId.Create(Self);
  //Self.MenuItemId.Caption := 'Личный чат с ' + TChatLine(self.FParentChatLine).ChatLineUsers[PDNode.DataUserId].DisplayNickName;
  Self.MenuItemId.Caption := fmInternational.Strings[I_PRIVATEWITH] + ' ' + PDNode.User.DisplayNickName;//Личный чат с
...
//========= Добавляем строчку меню: Создать линию
Self.MenuItemId := TMenuItemId.Create(Self);
Self.MenuItemId.Caption := fmInternational.Strings[I_CREATELINE];//Создать линию
...
Self.MenuItemId.Tag := 0;//короче надо в одном обработчике различать разные клики
Self.Items.Add(DynamicPopupMenu.MenuItemId);
//========= Добавляем строчку меню: Перейти к ресурсам компьютера
Self.MenuItemId := TMenuItemId.Create(Self);
...
Self.Items.Add(DynamicPopupMenu.MenuItemId);
//========= Добавляем строчку меню: Игнорировать все сообщения
//if AnsiCompareText(TChatLine(FParentChatLine).ChatLineUsers[FPDNode.DataUserId].ComputerName, TChatLine(FParentChatLine).LocalComputerName) <> 0 then
if AnsiCompareText(FPDNode.User.ComputerName, TChatLine(FParentChatLine).LocalComputerName) <> 0 then
  begin
  Self.MenuItemId := TMenuItemId.Create(Self);
...
//Begin DChat 1.0
//========= Добавляем строчку меню: [ от Плагинов ]

if FormMain.FSettings <> nil then
with FormMain do
begin
for i := 0 to FSettings.listBox2.Items.Count - 1 do
  ...
  if FSettings.listBox2.Items.Objects[i] <> nil then
    ...
    if FSettings.listBox2.Items.Objects[i] is TDChatClientServerPlugin then
      ...
      MenuNumb := 0;
      //вызываем функцию плагина, передаем ему команду 'GetMenuItem'
      //в ответ получаем строчку для меню
      MenuCaption := ClientServerPlugin.ExecuteCommand('GetMenuItem', Pchar(inttostr(MenuNumb)), 0);
      //вызываем функцию плагина с командой 'GetMenuItem' до тех пор, пока она > 0
      //таким образом плагин может вернуть несколько строк для контекстного меню TMenuItem
...
Self.MenuItemId.Tag := 0;//короче надо в одном обработчике различать разные клики
Self.Items.Add(DynamicPopupMenu.MenuItemId);
//========= Добавляем строчку меню: О пользователе
MenuItemId := TMenuItemId.Create(Self);
MenuItemId.Caption := fmInternational.Strings[I_USERINFO];//О пользователе
MenuItemId.OnClick := OnMenuClick;
...
procedure TDynamicVTHPopupMenu.AddRxTrayMenu(Sender: TComponent);
var
  MenuItemId: TMenuItemId;
  i, MenuNumb: integer;
  ClientServerPlugin : TDChatClientServerPlugin;
  MenuCaption: string;
begin
DynamicPopupMenu.Items.Clear;
DynamicPopupMenu.Images.Clear;
//Begin DChat 1.0
//========= Добавляем строчку меню: [ от Плагинов ]
...
//========= <Exit>
DynamicPopupMenu.MenuItemId := TMenuItemId.Create(Sender);
DynamicPopupMenu.MenuItemId.Caption := fmInternational.Strings[I_EXIT];
DynamicPopupMenu.MenuItemId.Tag := ONMENURxTrayExit;//короче надо в одном обработчике различать разные клики
DynamicPopupMenu.MenuItemId.OnClick := DynamicPopupMenu.OnMenuClick;
DynamicPopupMenu.Items.Add(DynamicPopupMenu.MenuItemId);
DynamicPopupMenu.MenuItemId.ImageIndex := DynamicPopupMenu.Images.AddMasked(TDreamChatImageLoader.GetImage(G_POPUP_EXIT).Bitmap[0],
                                                                            TDreamChatImageLoader.GetImage(G_POPUP_EXIT).Bitmap[0].TransparentColor);
//========= </Exit>
end;
...
var
  ...
  //DChat 1.0
begin
//создаем меню, появляющееся при нажатии на Приват в дереве
FParentChatLine := FormMain.GetActiveChatLine;
FPDNode := PDNode;
Self.Items.Clear;
Self.MenuItemId := TMenuItemId.Create(Self);
...
User := PDNode.User;
//Self.MenuItem.Caption := 'Войти в приват ' + TLineNode(User.ChatLinesList.Objects[PDNode.DataLineId]).DisplayLineName;
...
var
  ...
  p:TPoint;
begin
//создаем меню, появляющееся при нажатии на Линия в дереве
FParentChatLine := FormMain.GetActiveChatLine;
FPDNode := PDNode;
Self.Items.Clear;
Self.MenuItemId := TMenuItemId.Create(Self);
//User := TChatLine(self.FParentChatLine).ChatLineUsers[PDNode.DataUserId];
User := PDNode.User;
//Self.MenuItem.Caption := 'Войти в линию ' + TLineNode(User.ChatLinesList.Objects[PDNode.DataLineId]).DisplayLineName;
...
case TMenuItem(Sender).Tag of
  ...
  ONMENUTDebugMemo2SaveLog:
    ...
    //DChat 1.0 тут происходит обработка нажатия на строку меню, которую добавил плагин
    begin
...
//      end
      ...
      //            Memo2.Lines.Add('ExecuteCommand ShowForm');
              ...
              1:
                begin
                if (ClientServerPlugin <> nil) and (FPDNode <> nil) then
                  begin
//                  Randomize;
                  User := FormMain.GetMainLine.GetLocalUser;
                  InitString := '[Client]' + #13 +
//                                'ServerIP=127.0.0.1'+ #13 +
                                'ServerIP=' + FPDNode.User.IP + #13 +
                                'ServerPort=5557'+ #13 +
                                'NickName=' + User.NickName + #13 +
                                'RemoteNickName=' + FPDNode.User.DisplayNickName + #13 +
//                  временно генерим случайный порт, чтобы произошла ошибка связи
...
if ACanvas.ClipRect.Right > FormUI.Width then
  begin
  //если ширина Формы с инфой пользователя меньше, чем ширина меню
