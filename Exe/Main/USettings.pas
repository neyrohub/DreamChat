...
//DChat 1.0
  private
    ...
    DefUser:  TConfigChatUser;
       //Настройки вывода сообщений других пользователей чата
...
begin
//тут надо сделать чтобы конвертилось только в одну строну!
...
var
  ...
  CLine:  TChatLine;
begin
  TDreamChatConfig.SetProtoName('iChat');//пока этот параметр не используется
  ...
  if Length(ConfUsers) > 0 then
    for i := 0 to High(ConfUsers) do
      ...
      begin
        ...
        //если этот пользователь он-лайн, сразу же
        //применяем изменения в настройках его профиля
        begin
